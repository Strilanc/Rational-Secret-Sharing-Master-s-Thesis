...
namespace ThesisRationalSharingTest {
    ...
    public class ModIntPolynomialTest {
        ...
        public void ModIntPolynomialConstructorTest() {
            ...
            Assert.IsTrue(Polynomial<ModInt>.FromCoefficients(new[] { b + 4, b + 5, b + 6 }).ToString() == "6x^2 + 5x + 4 (mod 7)");
            Assert.IsTrue(Polynomial<ModInt>.FromCoefficients(new[] { c + 4, c + 5, c + 6 }).ToString() == "x^2 + 4 (mod 5)");
        }

        [TestMethod()]
        public void DivRemTest() {
            var c = new ModInt(0, 5);
            Polynomial<ModInt> f1 = Polynomial<ModInt>.FromCoefficients(new[] { c + 1, c + 1 });
            Polynomial<ModInt> f2 = Polynomial<ModInt>.FromCoefficients(new[] { c + 3, c + 2 });
            Polynomial<ModInt> p = f1 * f2;

            Assert.IsTrue(p.DivRem(f1).Item1 == f2);
            Assert.IsTrue(p.DivRem(f1).Item2 == Polynomial<ModInt>.FromCoefficients(new ModInt[0]));
            Assert.IsTrue(p.DivRem(f2).Item1 == f1);
            Assert.IsTrue(p.DivRem(f2).Item2 == Polynomial<ModInt>.FromCoefficients(new ModInt[0]));

            Assert.IsTrue(f1.DivRem(p).Item1 == Polynomial<ModInt>.FromCoefficients(new ModInt[0]));
            Assert.IsTrue(f1.DivRem(p).Item2 == f1);
            Assert.IsTrue(f2.DivRem(p).Item1 == Polynomial<ModInt>.FromCoefficients(new ModInt[0]));
            Assert.IsTrue(f2.DivRem(p).Item2 == f2);

            Assert.IsTrue(f2.DivRem(f1).Item1 == Polynomial<ModInt>.FromCoefficients(new[] { c+2 }));
            Assert.IsTrue(f2.DivRem(f1).Item2 == Polynomial<ModInt>.FromCoefficients(new[] { c+1 }));
        }
        ...
        public void EqualsTest() {
            var c = new ModInt(0, 5);
            Polynomial<ModInt> f1 = Polynomial<ModInt>.FromCoefficients(new[] { c + 1, c + 1 });
            Polynomial<ModInt> f2 = Polynomial<ModInt>.FromCoefficients(new[] { c + 2, c + 3 });
            Polynomial<ModInt> g1 = Polynomial<ModInt>.FromCoefficients(new[] { c + 1, c + 1 });
            Polynomial<ModInt> g2 = Polynomial<ModInt>.FromCoefficients(new[] { c + 2, c + 3 });
        ...
        public void EvaluateAtTest() {
            ...
            var zero = Polynomial<ModInt>.FromCoefficients(new ModInt[0]);
            var one = Polynomial<ModInt>.FromCoefficients(new ModInt[] { b+1 });
            var X = one << 1;
        ...
        public void FromInterpolationTest() {
            ...
            var s1 = new Point<ModInt>(b + 1, b + 2);
            var s2 = new Point<ModInt>(b + 2, b + 3);
            var s3 = new Point<ModInt>(b + 3, b + 5);
            var s4 = new Point<ModInt>(b + 4, b + 7);
            var s5 = new Point<ModInt>(b + 5, Polynomial<ModInt>.FromInterpolation(new[] { s1, s2, s3, s4 }).EvaluateAt(b+5));
            var s6 = new Point<ModInt>(b + 6, Polynomial<ModInt>.FromInterpolation(new[] { s1, s2, s3, s4 }).EvaluateAt(b + 6));

            // constant
            Assert.IsTrue(Polynomial<ModInt>.FromInterpolation(new[] { s1 }).EvaluateAt(b + 0) == s1.Y);
            Assert.IsTrue(Polynomial<ModInt>.FromInterpolation(new[] { s1 }).EvaluateAt(b + 95) == s1.Y);
            Assert.IsTrue(Polynomial<ModInt>.FromInterpolation(new[] { s2 }).EvaluateAt(b + 0) == s2.Y);
            Assert.IsTrue(Polynomial<ModInt>.FromInterpolation(new[] { s2 }).EvaluateAt(b + -1) == s2.Y);

            // line
            Assert.IsTrue(Polynomial<ModInt>.FromInterpolation(new[] { s1, s2 }).EvaluateAt(b + 0) == 1);
            Assert.IsTrue(Polynomial<ModInt>.FromInterpolation(new[] { s1, s2 }).EvaluateAt(b + -1) == 0);
            Assert.IsTrue(Polynomial<ModInt>.FromInterpolation(new[] { s1, s2 }).EvaluateAt(b + 6) == 7);
        }

        [TestMethod()]
        public void ArithmeticTest() {
            var one = Polynomial<ModInt>.FromCoefficients(new[] { new ModInt(1, 11) });
            var X = one << 1;
